 ...
 */
@SpringBootTest @AutoConfigureMockMvc class SbomApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("在管组件"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanSubmitProductionReport()throws Exception{mvc.perform(post("/api/shopfloor/work-orders/1/reports").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"operationName\":\"组件处置\",\"goodQty\":2,\"defectQty\":1,\"remark\":\"证明已归档\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.message").value("组件处置反馈已提交")).andExpect(jsonPath("$.data.completedQty").value(18));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanEvaluateComponentRisk()throws Exception{mvc.perform(post("/api/shopfloor/component-risk").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"productName\":\"支付服务\",\"componentName\":\"demo-lib\",\"version\":\"1.2.3\",\"cvss\":9.8,\"exploitKnown\":true,\"directDependency\":true,\"license\":\"AGPL-3.0\",\"approvedLicense\":false,\"fixAvailable\":true}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.policyDecision").value("BLOCK")).andExpect(jsonPath("$.data.severity").value("CRITICAL"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
